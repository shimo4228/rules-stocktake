...
- Phase 4 Consolidation now confirms candidates **one by one** (evidence first, then `[y/n/skip]`), explicitly banning bulk approval — the confirm-each design borrowed from config-gc. The user can stop at any point; `skip` records the verdict in the ledger unactioned. This matters doubly for rules-stocktake because approved edits are applied in-session.
